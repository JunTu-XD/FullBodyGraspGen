...
def save_dataloader(grabpose):
    ...
    for it, dorig in enumerate(grabpose.ds_train):
        ...
        with torch.no_grad():
            input,output, mu_train,var_train  = grabpose.full_grasp_net(**dorig)
        input_set.append(input)
        output_set.append(output)
        mu_set.append(mu_train)
        var_set.append(var_train)
        # if it == 2:
        #     break
        
    input_set = torch.cat(input_set,dim=0)
    output_set= torch.cat(output_set,dim=0)
    mu_set    = torch.cat(mu_set,dim=0)
    var_set   = torch.cat(var_set,dim=0)
    label_set = torch.cat([output_set,mu_set,var_set],dim=1)
    
    ############################eval data############################
    data = grabpose.ds_val 
    eval_input_set = []
    eval_output_set=[]
    eval_mu_set = []
    eval_var_set = []
    ...
    with torch.no_grad():
        for dorig in data:
            ...
            test_input, test_output, test_mu,test_var  = grabpose.full_grasp_net(**dorig)
            eval_input_set.append( test_input)
            eval_output_set.append(test_output)
            eval_mu_set.append(test_mu)
            eval_var_set.append(test_var)
            i+=1
            # if it == 2:
            #     break
            #breakpoint()
    eval_input_set = torch.cat(eval_input_set,dim=0)
    eval_output_set= torch.cat(eval_output_set,dim=0)
    eval_mu_set    = torch.cat(eval_mu_set,dim=0)
    eval_var_set   = torch.cat(eval_var_set,dim=0)
    eval_label_set = torch.cat([eval_output_set,eval_mu_set,eval_var_set],dim=1)
    
    save_data_dict ={'train_inp':input_set,'train_label':label_set,
                     'eval_inp':eval_input_set,'eval_':eval_label_set}
    
    torch.save(save_data_dict,'./female_data_dict.pt')
